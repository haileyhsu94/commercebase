...
| AI proxy | Laravel backend proxies Codex API calls for Aeris |
...
- **Decision:** Aeris will route through the Laravel backend (not direct frontend calls) — keeps Codex API keys server-side
...
| **Aeris routing** | Backend proxy | Frontend calls Laravel API, which proxies to Codex API. Keeps API keys server-side. |
